...
To ensure the **Pocketbase** application container is ready for use immediately upon startup the `entrypoint.sh` bootstraps it with a superuser and directory structure on first boot.
...
**Developer shortcuts:** Se Makefile.
...
Too verify the full integration (Proxy, Mail-trap, and Backend), simply run: 
